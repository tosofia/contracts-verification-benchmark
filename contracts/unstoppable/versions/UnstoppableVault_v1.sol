 ...
 */
contract UnstoppableVault is IERC3156FlashLender, IERC3156FlashBorrower, ReentrancyGuard, Owned, ERC4626, Pausable {
    ...
    function onFlashLoan(address initiator, address token, uint256 amount, uint256 fee, bytes calldata)
        ...
        returns (bytes32)
    {
        if (initiator != address(this) || msg.sender != address(this) || token != address(this.asset())) {
            revert UnexpectedFlashLoan();
        }

        ERC20(token).approve(address(this), amount + fee);
