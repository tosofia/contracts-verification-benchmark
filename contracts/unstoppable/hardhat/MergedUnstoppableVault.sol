...
import {SafeTransferLib, ERC4626, ERC20} from "./ERC4626v2.sol";
 ...
 */
contract MergedUnstoppableVault is IERC3156FlashLender, IERC3156FlashBorrower, ReentrancyGuard, Owned, ERC4626, Pausable {
    ...
    function onFlashLoan(address initiator, address _token, uint256 amount, uint256 fee, bytes calldata)
        ...
        returns (bytes32)
    {
        if (initiator != address(this) || msg.sender != address(this) || _token != address(asset)) {
            revert UnexpectedFlashLoan();
        }

        ERC20(_token).approve(address(this), amount + fee);
